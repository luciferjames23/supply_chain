      ...
      {subTab === 'orders' && (
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {orders.filter(o => o.order_id?.toLowerCase().includes(search.toLowerCase())).map((item) => (
      ...
      {subTab === 'shipments' && (
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {shipments.filter(s => s.shipment_id?.toLowerCase().includes(search.toLowerCase())).map((item) => (
      ...
      {subTab === 'inventory' && (
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {inventory.filter(i => i.product_id?.toLowerCase().includes(search.toLowerCase())).map((item, idx) => (
                <tr key={idx} onClick={() => onItemClick(item, 'Inventory Detail')}>
                  <td><strong style={{ color: 'var(--accent-cyan)' }}>{item.product_id}</strong></td>
                  <td>{item.product_name}</td>
                  <td>{item.warehouse_id}</td>
                  <td><strong>{item.available_quantity?.toLocaleString()}</strong></td>
                  <td>{item.reorder_point?.toLocaleString()}</td>
                  <td>{item.safety_stock?.toLocaleString()}</td>
                  <td><span className={`badge ${(item.stock_status || 'OPTIMAL').toLowerCase()}`}>{item.stock_status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {subTab === 'products' && (
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {products.filter(p => p.product_id?.toLowerCase().includes(search.toLowerCase()) || p.product_name?.toLowerCase().includes(search.toLowerCase())).map((item) => (
      ...
      {subTab === 'suppliers' && (
        <div className="table-container">
          <table className="data-table">
            ...
            <tbody>
              {suppliers.filter(s => s.supplier_id?.toLowerCase().includes(search.toLowerCase()) || s.supplier_name?.toLowerCase().includes(search.toLowerCase())).map((item) => (
